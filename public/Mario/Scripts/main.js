...
var Level = Base.extend({
	...
	load: function (level) {
		...
		this.setSize(level.width * 32 * 2, level.height * 32);
	...
	playMusic: function (para) {

		// this.audio[para].play();
	},
	stopMusic: function (label) {
	...
	setParallax: function (x) {
		this.setPosition(x * 2, this.y);
		this.world.parent().css('background-position', '-' + Math.floor(x / 3) + 'px -0px');
	},
});
 ...
 * -------------------------------------------
 */
var RightPipeGrass = Ground.extend({
	init: function (x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
...
var LeftPipeGrass = Ground.extend({
	init: function (x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
...
var RightPipeSoil = Ground.extend({
	init: function (x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
...
var LeftPipeSoil = Ground.extend({
	init: function (x, y, level) {
		var blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
...
var Mario = Hero.extend({
	...
	setPosition: function (x, y) {
		...
		var r = this.level.width - screenWidth;
		var w = (this.x <= 210) ? 0 : ((this.x * 2 >= this.level.width - 460) ? r / 2 - 10 : r / (this.level.width - 860) * (this.x - 210));
		this.level.setParallax(w);
		if (this.onground && this.x >= this.level.width / 2 - 200) {
			window.showfirework();
			this.victory();
		}
	},
	...
	walk: function (reverse, fast) {
		this.vx = constants.walking_v * (fast ? 1.5 : 1) * (reverse ? - 1 : 1);
	},
	walkRight: function () {
		if (this.state === size_states.small) {
...
$(document).ready(function () {
	...
	for (var i = 0; i < musics.length; i++) {
		mySounds[i] = new sound('' + AUDIOPATH + musics[i]);
	}
